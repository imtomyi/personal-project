...
function TodoItem({
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    touchRef.current = { startX: touch.clientX, startY: touch.clientY, started: false };
    setSwipeX(0);
    setIsSwiping(false);
  }, []);
  ...
  }, [swipeX, onDelete, onUpdate, todo.id, todo.is_completed]);
  ...
  return (
    <div
      ...
      onTouchStart={!isSectionHeader && !isEditing ? handleTouchStart : undefined}
      onTouchMove={!isSectionHeader && !isEditing ? handleTouchMove : undefined}
      onTouchEnd={!isSectionHeader && !isEditing ? handleTouchEnd : undefined}
    >
      ...
      <div
        className="relative bg-white p-4 dark:bg-gray-800"
        style={{
      ...
      <div className="flex items-start gap-3">
          ...
          {isEditing ? (
            ...
            >
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {/* 마감 긴급도 표시 — 3일 전부터 점진적 색상 */}
                {urgency && (
                  <span className={`text-[10px] font-medium ${urgency.textColor}`}>
                    {urgency.label}
                  </span>
                )}
              </div>
        ...
        <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {/* Assign dropdown — team plan only */}
          {isTeam && (
            <select
